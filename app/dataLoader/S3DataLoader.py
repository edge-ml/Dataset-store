...
class S3DataLoader(BaseDataLoader):
    def __init__(self):
        self.s3 = boto3.resource('s3', endpoint_url=S3_URL, aws_access_key_id=S3_ACCESS_KEY, aws_secret_access_key=S3_SECRET_KEY)

    def load_series(self, id):
        obj = self.s3.Object(S3_BUCKET_NAME, id)
    ...
    def save_series(self, id, time_arr, data_arr):
        ...
        buffer.seek(0)
        self.s3.Object(S3_BUCKET_NAME, id).put(Body=buffer.getvalue())
        print("Series saved to S3 successfully.")


def delete(self, id):
    self.s3.Object(S3_BUCKET_NAME, id).delete()
