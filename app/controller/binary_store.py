...
class BinaryStore():
    ...
    def _appendValues(self, time, data):
        ...
        sampling_rate_mean = np.nan_to_num(np.mean(time_diff)) if len(time_diff) > 0 else np.NaN
        sampling_rate_var = np.nan_to_num(np.var(time_diff)) if len(time_diff) > 0 else np.NaN
