...
@router.get("/")
def get_project_labelings(project: str = Header()):
    res = getProjectLabelings(project)
    return Response(json.dumps(res, cls=JSONEncoder), media_type="application/json")

@router.post("/")
async def create_labeling(body: Request, project: str = Header()):
    body = await body.json()
    res = createLabeling(project, body)
    return Response(json.dumps(res, cls=JSONEncoder), media_type="application/json")

@router.put("/{id}")
async def update_labeling(id, body: Request, project: str = Header()):
    body = await body.json()
    res = updateLabeling(project, id, body)

@router.delete("/{id}")
async def delete_labeling(id, project: str = Header()):
